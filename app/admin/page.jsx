  ...
  useEffect(() => {
    (async () => {
      const [d, m, s, sp, rc] = await Promise.all([
        ...
        supabase.from('members').select('name, role_title, updated_at').order('updated_at', { ascending: false }).limit(5),
      ]);
...
function TeamSection({ toast }) {
  ...
  const selectDomain = async (domain) => {
    ...
    const [leadRes, membersRes] = await Promise.all([
      ...
      supabase.from('members').select('*').eq('domain_id', domain.id).order('display_order'),
    ]);
  ...
  const saveMemberPhoto = async (id, url) => {
    await supabase.from('members').update({ photo_url: url }).eq('id', id);
