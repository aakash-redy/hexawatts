            ...
            <div className="p-6 md:p-8">
                  ...
                  <button
                    ...
                    className={`py-3 rounded-xl font-bold text-sm transition-all duration-300 ${
                      selectedAmount === preset.value
                        ? "bg-[#FFC600] text-black shadow-[0_0_20px_rgba(255,198,0,0.3)]"
                        : "bg-white/[0.03] text-white/70 border border-white/[0.08] hover:border-[#FFC600]/30 hover:text-white"
                    }`}
                  >
                    {preset.label}
                  </button>
                ...
                <div className="relative">
                  <input
                    ...
                    className={`w-full py-3 rounded-xl font-bold text-sm text-center transition-all duration-300 ${
                      customAmount
                        ? "bg-[#FFC600] text-black shadow-[0_0_20px_rgba(255,198,0,0.3)] border-[#FFC600]"
                ...
                >
                  <div>
                    ...
                    <input type="text" name="name" value={formData.name} onChange={handleFormChange} placeholder="Your full name"
                      className="w-full bg-white/[0.03] border border-white/[0.08] rounded-xl px-4 py-3 text-white text-sm placeholder:text-white/20 focus:border-[#FFC600]/40 focus:outline-none transition-all" />
                  </div>
                  ...
                  <div>
                    ...
                    <textarea name="message" value={formData.message} onChange={handleFormChange} placeholder="Write a message of support..." rows={2}
                      className="w-full bg-white/[0.03] border border-white/[0.08] rounded-xl px-4 py-3 text-white text-sm placeholder:text-white/20 focus:border-[#FFC600]/40 focus:outline-none transition-all resize-none" />
                  </div>
                  ...
                  <label className="flex items-center gap-3 cursor-pointer group">
                    <div className="relative">
                      ...
                      <div className={`w-5 h-5 rounded-md border-2 transition-all duration-300 flex items-center justify-center ${
                        formData.isAnonymous ? "bg-[#FFC600] border-[#FFC600]" : "bg-white/[0.03] border-white/[0.15] group-hover:border-[#FFC600]/30"}`}>
              ...
              {finalAmount > 0 && (
                <motion.button
                  ...
                  className="w-full mt-6 bg-[#FFC600] text-black py-4 rounded-xl font-black text-base uppercase tracking-wider disabled:opacity-30 disabled:cursor-not-allowed hover:shadow-[0_8px_30px_rgba(255,198,0,0.4)] transition-all duration-300"
                >
          ...
          {step === "payment" && (
            <div className="p-6 md:p-8">
              ...
              <p className="text-white/40 text-sm mb-6">
                Amount: <span className="text-[#FFC600] font-bold">₹{finalAmount.toLocaleString("en-IN")}</span>
              </p>
              ...
              <div className="space-y-3 mb-6">
                {[
                  ...
                  <button key={method.id} onClick={() => handleSimulatePayment(method.label)} disabled={isSubmitting}
                    className="w-full flex items-center gap-4 bg-white/[0.03] border border-white/[0.08] hover:border-[#FFC600]/30 rounded-xl p-4 transition-all duration-300 group disabled:opacity-50 disabled:cursor-not-allowed">
                    <span className="text-2xl">{method.icon}</span>
                    <div className="text-left flex-1">
                      <p className="text-white font-bold text-sm group-hover:text-[#FFC600] transition-colors">{method.label}</p>
                      <p className="text-white/30 text-[10px]">{method.desc}</p>
                    </div>
                    <span className="text-white/20 group-hover:text-[#FFC600] group-hover:translate-x-1 transition-all">→</span>
                  </button>
                ))}
              </div>
              ...
              {isSubmitting && (
                <div className="text-center py-4">
                  <div className="inline-flex items-center gap-3">
                    <div className="w-5 h-5 border-2 border-[#FFC600]/30 border-t-[#FFC600] rounded-full animate-spin" />
          ...
          {step === "success" && (
            <div className="p-6 md:p-8 text-center">
              <motion.div
                ...
                className="w-20 h-20 bg-[#FFC600]/10 border border-[#FFC600]/30 rounded-full flex items-center justify-center mx-auto mb-6"
              >
                <svg className="w-10 h-10 text-[#FFC600]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <p className="text-white/50 text-sm mb-2">
                Your contribution of <span className="text-[#FFC600] font-bold">₹{finalAmount.toLocaleString("en-IN")}</span> has been received.
              </p>
              {simulatedPayment && (
              ...
              <button onClick={handleClose}
                className="w-full bg-[#FFC600] text-black py-4 rounded-xl font-black text-sm uppercase tracking-wider hover:shadow-[0_8px_30px_rgba(255,198,0,0.4)] transition-all duration-300">
                Close
              </button>
            </div>
          )}
